...
\texttt{MEANINGFUL\_RANK\_GAIN} of the committed policy --- the paired-bootstrap 95\% percentile
...
\medskip\noindent\textbf{What the executable comparison actually is.} The gate is the
binary64 comparison \texttt{rank\_gain >= MEANINGFUL\_RANK\_GAIN}, not an exact real-number test of
$R(\mathrm{M})-R(\mathrm{L})\ge 1/6$, and at the boundary the two are not equivalent. The stored
threshold is the double nearest to $1/6$, $0.16666666666666666$. A rank configuration that is
exactly $1/6$ in rational arithmetic --- ResEnc-L ahead on three components, tied on one, behind on
two --- is computed as a difference of averaged ranks and evaluates to approximately
$0.16666666666666652$, about $1.4\times10^{-16}$ below the stored threshold, so that nominal
boundary case is rejected. The disclosure is made because the manuscript quotes the nominal value.
It did not affect the recorded architecture decision: the observed statistic was nowhere near the
boundary, the fold-0 rank gain being $-0.333$ against a required $+1/6$, and no other declared
condition was met either. We did not modify the frozen policy code, and no result changed.
